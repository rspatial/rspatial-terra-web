...
set.seed(1963)
bgcells <- spatSample(r, 5000, "random", na.rm=T, cells=TRUE)
bg <- xyFromCell(r, bgcells)
```
...
```{r}
e <- ext(-80, -53, -39, -22)
bg2 <- spatSample(r, 100, "random", na.rm=TRUE, xy=TRUE, ext=e)
plot(!is.na(r))
...
set.seed(999)
samp1 <- spatSample(pol, 250, "random")
# get cells
...
Note that the blue points are not all within the polygons (circles), as they now represent the centers of the selected cells from mask. We could choose to select only those cells that have their centers within the circles, using the `intersect` method.
