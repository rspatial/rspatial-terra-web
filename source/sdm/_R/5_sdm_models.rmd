...
partialResponse(bc, presvals[,c('bio1', 'bio5', 'bio12')])
```
...
ff <- list.files(file.path(system.file(package="predicts"), "ex"), pattern="tif$", full.names=TRUE )
predictors <- rsat(ff)
names(predictors)
...
Below we show how you can compute these, and other statistics more conveniently, with the `evaluate` function in the dismo package. See `?evaluate` for info on additional evaluation measures that are available.
...
for (i in 1:k) {
	...
	test <- pres[group == i,]
	bc <- bioclim(train)
	e[[i]] <- pa_evaluate(p=test, a=back, bc)
}	
```
...
```{r, sdm45b}
file <- file.path(system.file(package="predicts"), "ex/bradypus.csv")
bradypus <- read.table(file,  header=TRUE,  sep=',')
bradypus <- bradypus[,-1]
presvals <- extract(predictors, bradypus)
set.seed(0)
backgr <- spatSample(predictors, 500, na.rm=TRUE)
...
pa_evaluate(bc, p=pres_test, a=back_test, x=predictors)
pa_evaluate(bc, p=pres_test_pwd, a=back_test_pwd, x=predictors)
```
