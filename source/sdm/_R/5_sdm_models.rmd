...
Models that are implemented in `predicts` do not use a formula (and most models only take presence points). The `envelope` (a.k.a. "Bioclim") is an example. It only uses presence data, so we use 'presvals' instead of 'sdmdata'.
...
In most cases, howver, the purpose is SDM is to create a map of suitability scores. We can do that by providing the predict function with a Raster* object and a model object. As long as the variable names in the model object are available as layers (layerNames) in the SpatRaster object.
...
Below we show how you can compute these, and other statistics more conveniently, with the `evaluate` function in the dismo package. See `?pa_evaluate` for info on additional evaluation measures that are available.
...
We can extract several things from the objects in lise `e1, but let's restrict ourselves to the AUC values and the "maximum of the sum of the sensitivity (true positive rate) and specificity (true negative rate)" threshold "spec_sens" (this is sometimes uses as a threshold for setting cells to presence or absence).
...
pa_evaluate(p=pres_test, a=back_test, model=bc, data=predictors)
pa_evaluate(p=pres_test_pwd, a=back_test_pwd, model=bc, data=predictors)
```
