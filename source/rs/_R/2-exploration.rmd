...
__Question 1__: *Use the plotRGB function with the multi-band (11 bands) `landsat` image to create a true and false color composite (hint remember the position of the bands in the image).* 
...
At this stage we may want to save the raster to disk with `writeRaster`. Multiple file types are supported. We will use the commonly used GeoTiff format. While the layer order is preserved, layer names are unfortunately lost in the GeoTiff format.
