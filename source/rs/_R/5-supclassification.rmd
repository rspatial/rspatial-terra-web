...
Todo: show how can we change legend labels in terra::plot
Todo: show how to get coordinates of the sampled cells
...
Now we have our trained classification model (`cartmodel`), we can use it to make predictions, that is, to classify all cells in the `landsat5` SpatRaster.

**Important** The layer names in the Raster object to be predicted should exactly match those expected by the trained model. This will be the case if the same SpatRaster object was used (via extract) to obtain the values to fit the model. Otherwise you need to specify the matching names.
