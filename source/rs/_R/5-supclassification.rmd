...
```{r prediction, fig.height=6, fig.width=9}
classified <- predict(landsat, cartmodel, na.rm = TRUE)
classified
plot(classified)
```

Observe that there are `r nlyr(classified)` layers in the `classified` object, each of the layer representing the probability of a particular LULC class. Bewlow, we make a single layer SpatRaster showing, for each grid cell, the LULC class with the highest probability.  
...
```{r combine-results2, fig.width=8, fig.height=7}
