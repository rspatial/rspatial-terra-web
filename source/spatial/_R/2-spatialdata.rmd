...
Recent packages in R that define such spatial data structures include `terra` and `sf`. These packages replace a set of older packages including `sp`, `raster`, `rgdal` and `rgeos`.

We mostly use the `terra` package in these materials. You can install the latest released version of *terra* from CRAN with `install.packages(`terra`) and the development version from [github](https://www.github.com/rspatial/terra). For the same material with the *raster* and *sp* packages, go [here](https://rspatial.org/spatial/). 