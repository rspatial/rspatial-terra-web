...
```{r}
wr <- relate(pols, relation="rook")
rownames(wr) <- colnames(wr) <- pols$AU_NAME
wr[1:5,1:5]
``` 

Anotehr way, to get the IDs

```{r}
wr <- relate(v, relation="rook", wide=FALSE, symmetrical=TRUE)
wr <- wr[wr[,3]==1, ]
head(wr)
``` 
...
```{r}
wq <- adjacent(v, "queen", symmetrical=TRUE)
``` 
...
After doing this 'by hand', now let's use the terra package to compute Moran's *I* and do a significance test. 
