...
```{r, p4-2, fig.width=9, fig.height=2.5}
library(terra)
...
We can use algebraic exprssions with SpatRaster objects 
```{r}
Z <- 2*X + 3*Y
```
...
Plot the result.
```{r, p4-3, fig.cap="", fig.width=9, fig.height=5}
plot(Z)
text(Z, cex=.75)
...
We can call function `f` as many times as we like, below I use it four times. Note that the functions has no arguments, but we still need to use the parenthesis `f()` to distinguish it from `f`, the function definition.

```{r, p4-6, fig.width=10, fig.height=5}
set.seed(777)
...
xy <- cbind(x,y)
p <- rasterize(vect(xy), r, fun=length, background=0)
plot(p)
plot(quads, add=TRUE, border="gray")
points(x, y, pch=20)
```
