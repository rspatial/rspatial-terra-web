...
We will be using the `sp`, `spdep` and related packages. And we transform the SpatVector data to a Spatial* object.

```{r, locasp}
auck <- as(auck, "Spatial")
``` 
...
spt <- as(terra::project(sp, alb), "Spatial")
vctst <- terra::project(cts, alb)
ctst <- as(vctst, "Spatial")
```
